    ...
    SmohanMusic.prototype = {
        ...
        setVolume : function(volume){
            var v = this.musicDom.volume,
                h = v.volumeEventer.offsetWidth || 100;  //隐藏元素的高度不好获取，这里设置visible来实现同样的效果
            if(volume <= 0) volume = 0;
            if(volume >= 1) volume = 1;
            this.audioDom.volume = volume;
            var currentHeight = h * volume;
            v.volumeCurrent.style.width = currentHeight + 'px';
            /*v.volumeCtrlBar.style.left = currentHeight-4 + 'px';*/
        ...
        action : function(){
            ...
            document.addEventListener('click',function(event){
                ...
                if((target !== $('.mlist-btn')) && target.parentNode.parentNode.parentNode !== $('.m-music-list-wrap.on')){
                    //当触发事件的元素不在音量元素范围里时关闭
                    btn.mlistbtn.setAttribute('data-state',"false");
                    removeClass(me.musicDom.listWrap,"on");
                }
            },false);
            //音量控制
            ...
            //列表收起/打开
            btn.mlistbtn.addEventListener('click',function(){
                var btnw = this;
                if (btnw.getAttribute('data-state')=="true") {
                    btnw.setAttribute('data-state',"false");
                    removeClass(me.musicDom.listWrap,"on");
                }else{
                    btnw.setAttribute('data-state',"true");
                    addClass(me.musicDom.listWrap,"on");
                }
        ...
        init : function(){
            ...
            this.musicDom = {
                ...
                button  : {
                    mlistbtn :$('.grid-music-container .mlist-btn'), //列表开关
